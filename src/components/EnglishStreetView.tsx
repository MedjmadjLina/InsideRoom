...
import { STREET_Y, FLOOR_H, floorBottomY } from "./BuildingShell";
...
// ── English building palette ─────────────────────────────────────────────────
const OPPOSITE_BRICKS = [
  "#b84c38", // warm red London brick (like ours)
  "#c86048", // slightly lighter red
  "#9e3c2c", // darker Victorian red
  "#d4a882", // buff/yellow stock brick
  "#c8b898", // London stock pale
  "#e8e0d0", // white stucco
  "#f0e8d8", // cream painted
];
const STONE_WHITE = "#ece6dc";

// ── Atmospheric tint helper ───────────────────────────────────────────────────
function atmosphericColor(baseColor: string, depth: number, fogColor: string): string {
  const d = Math.max(0, depth - 8) / 60;      // 0 = near, 1 = very far
...
// ===========================================================================
// STREET SURFACE — pavement + road
// ===========================================================================
function StreetSurface({ roomWidth, facadeZ }: { roomWidth: number; facadeZ: number }) {
  const streetW  = Math.max(roomWidth + 60, 180); // keep coverage with wide exterior camera angles
  const pavement = 2.2;               // width of our pavement
  const roadW    = 8.5;
  const farPave  = 2.0;
  ...
  return (
    <group position={[0, STREET_Y, 0]}>
      ...
      <mesh position={[0, 0.055, facadeZ - pavement]}>
        ...
        <meshStandardMaterial color="#c8c0b8" roughness={0.90} />
      </mesh>
      ...
      {/* Ground plane (extends back far — catches sky reflection) */}
        ...
        Array.from({ length: b.winCols }, (_, ci) => {
          ...
          return (
            <group key={`${fi}-${ci}`} position={[wx, wy, b.d / 2 + 0.004]}>
              {/* Stone surround */}
              <mesh>
                <boxGeometry args={[winW + 0.12, winH + 0.10, 0.018]} />
...
// ===========================================================================
// GENERATE OPPOSITE BUILDING ROW DATA
// ===========================================================================
function generateOppositeRow(
  ...
  while (x < spanW / 2) {
    const w      = 4 + rng() * 6;           // 4—10m wide
    const d      = 3 + rng() * 4;           // 3—7m deep
    const floors = 3 + Math.floor(rng() * 4); // 3—6 floors
...
// ===========================================================================
// SIMPLE TREES (London plane / linden style)
// ===========================================================================
function StreetTree({ x, z }: { x: number; z: number }) {
  const trunkH = 2.4;
  const canH   = 2.2;
  const canR   = 1.3;
  ...
  return (
    <group position={[x, STREET_Y, z]}>
      ...
      <mesh position={[0, trunkH + canH * 0.42, 0]} castShadow receiveShadow>
...
// STREET LAMPS
// ===========================================================================
function StreetLamp({ x, z, isOn }: { x: number; z: number; isOn: boolean }) {
  return (
    <group position={[x, STREET_Y, z]}>
      ...
      <mesh position={[0.38, 4.25, 0]} rotation={[0, 0, -Math.PI * 0.16]} castShadow>
        <cylinderGeometry args={[0.02, 0.028, 0.9, 8]} />
        <meshStandardMaterial color="#2b2724" roughness={0.48} metalness={0.76} />
      </mesh>
      <mesh position={[0.72, 4.18, 0]} castShadow>
...
// ===========================================================================
// PARKED CARS (simple stylised boxes)
// ===========================================================================
  ...
  return (
    <group position={[x, STREET_Y + 0.7, z]} rotation={[0, Math.PI / 2, 0]}>
      ...
      <mesh position={[0, 0.72, 0.08]}>
        <boxGeometry args={[1.55, 0.50, 2.05]} />
      ...
      {([-0.75, 0.75] as number[]).map((side) =>
        ([-1.3, 1.3] as number[]).map((front) => (
          <mesh key={`${side}-${front}`} position={[side, -0.55, front]} rotation={[Math.PI / 2, 0, 0]}>
            ...
            <meshStandardMaterial color="#1a1a1a" roughness={0.90} metalness={0} />
          </mesh>
        ))
      )}
    </group>
...
// ===========================================================================
// FAR SKYLINE — simplified silhouette blocks for depth
// ===========================================================================
...
// ROOT COMPONENT
// ===========================================================================
export default function EnglishStreetView() {
  ...
  const facadeZ   = -room.length / 2;         // z of our building's facade
  const nearRow = useMemo(
    () => generateOppositeRow(room.width, facadeZ, 13.5, 42),
    [room.width, facadeZ],
  );
  const midRow = useMemo(
    () => generateOppositeRow(room.width, facadeZ, 29, 77),
    [room.width, facadeZ],
  );

  // Tree positions along pavement of opposite side
  const trees = useMemo(() => {
  ...
  // Cars parked on near side of road
  const cars = useMemo(() => {
  ...
  return (
    <group>
      ...
      {nearRow.map((b, i) => (
        <OppositeBuilding key={`near-${i}`} b={b} fogColor={fogColor} />
      ))}
      {midRow.map((b, i) => (
        <OppositeBuilding key={`mid-${i}`} b={b} fogColor={fogColor} />
      ))}

      {/* ── Street trees on far pavement ── */}
      {trees.map((t, i) => <StreetTree key={i} x={t.x} z={t.z} />)}

      {/* ── Street lamps ── */}
      {streetLamps.map((lamp, i) => (
        <StreetLamp key={i} x={lamp.x} z={lamp.z} isOn={lampsOn} />
      ))}

      {/* ── Parked cars ── */}
      {cars.map((c, i) => <ParkedCar key={i} x={c.x} z={c.z} color={c.color} />)}
